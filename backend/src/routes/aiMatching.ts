...
router.get('/config', protect, authorize('admin'), async (_req:AuthRequest,res:Response)=>{ const cfg=await getConfig(); res.json({success:true, config:{...cfg.toObject(), apiKeys: cfg.apiKeys ? { openai: cfg.apiKeys.openai ? '••••'+cfg.apiKeys.openai.slice(-4) : '', gemini: cfg.apiKeys.gemini ? '••••'+cfg.apiKeys.gemini.slice(-4) : '' } : {}}}); });
router.put('/config', protect, authorize('admin'), async (req:AuthRequest,res:Response)=>{
